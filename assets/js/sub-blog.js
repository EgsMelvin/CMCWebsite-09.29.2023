$(function () {
    ...
    function addToBodybyId(newsletter) {
        ...
        var trElements = tempElement.getElementsByTagName('tr');
        for (var i = trElements.length - 1; i >= 0; i--) {
          ...
          if (
            (language === 'en' && tdText.includes('Want to change how you receive these emails?')) ||
            (language === 'fr' && tdText.includes('Voulez-vous changer les modalités de réception de ces emails?')) ||
            (language === 'de' && tdText.includes('Möchten Sie Ihre Newsletter-Einstellungen ändern?')) ||
            (language === 'it' && tdText.includes('Vuoi cambiare la modalità di ricezione di queste e-mail?'))
          ) {
            tr.parentNode.removeChild(tr);
          }
        }

        MessageHTML = tempElement.innerHTML;
    ...
    window.addEventListener('DOMContentLoaded', function() {
      ...
      for (var i = 0; i < tdElements.length; i++) {
        ...
        if (td.style.fontSize === '12px' && tdText.includes('Want to change how you receive these emails?')) {
